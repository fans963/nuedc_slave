...
USBD_DEVICE_DEFINE(usbd_ctx, DEVICE_DT_GET(DT_NODELABEL(usbotg_hs)), 0x1209,
                   0x0001);

USBD_DESC_LANG_DEFINE(usbd_lang);
USBD_DESC_MANUFACTURER_DEFINE(usbd_mfr, "NUEDC");
...
static int bulk_request(struct usbd_class_data *c_data, struct net_buf *buf,
                        int err) {
  ...
  if (bi->ep == BULK_OUT_EP) {
    if (g_bulk)
      g_bulk->on_rx_data(buf->data, buf->len);
    net_buf_unref(buf);
...
static void bulk_disable(struct usbd_class_data *c_data) {
  atomic_set(&g_enabled, 0);
}

static void *bulk_get_desc(struct usbd_class_data *c_data,
...
BulkDevice::BulkDevice() : decoder_(*this) {}
...
int BulkDevice::init() {
  ...
  usbd_add_configuration(&usbd_ctx, USBD_SPEED_FS, &usbd_fs_config);
  usbd_add_configuration(&usbd_ctx, USBD_SPEED_HS, &usbd_hs_config);

  usbd_register_class(&usbd_ctx, "bulk_class", USBD_SPEED_FS, 1);
  usbd_register_class(&usbd_ctx, "bulk_class", USBD_SPEED_HS, 1);

  int ret = usbd_init(&usbd_ctx);
  if (ret < 0)
    return ret;
...
void BulkDevice::on_rx_data(const uint8_t *data, size_t len) {
...
void BulkDevice::try_transmit() {
  ...
  for (auto *c : cans_)
    if (c)
      collect_writer(c->uplink_writer());
  for (auto *u : uarts_)
    if (u)
      collect_writer(u->uplink_writer());
  for (auto *s : spis_)
    if (s)
      collect_writer(s->uplink_writer());
  for (auto *i : i2cs_)
    if (i)
      collect_writer(i->uplink_writer());
  for (auto *m : imus_)
    if (m)
      collect_writer(m->uplink_writer());
  for (auto *a : adcs_)
    if (a)
      collect_writer(a->uplink_writer());
  for (auto *q : qdecs_)
    if (q)
      collect_writer(q->uplink_writer());
...
void BulkDevice::on_frame(const uint8_t *data, size_t len) {
  ...
  msg::dispatch_downlink(
      data, len, c0->downlink_buf(),
      cans_[1] ? cans_[1]->downlink_buf() : c0->downlink_buf(),
      u0->downlink_buf(),
      uarts_[1] ? uarts_[1]->downlink_buf() : u0->downlink_buf(),
      ...
      // PID config callback
      [this](uint8_t id, float kp, float ki, float kd,
             float out_min, float out_max, float int_min, float int_max, bool reset) {
          if (id < motors_.size() && motors_[id]) {
