...
int Qdec::init()
{
    if (!device_is_ready(dev_)) return -EIO;

    // Read initial count
    if (sensor_sample_fetch(dev_) < 0) return -EIO;
...
void Qdec::on_timer()
{
    ...
    if (configured_ && counts_per_rev_ > 0) {
        velocity_ = static_cast<float>(delta) * TWO_PI
                  / static_cast<float>(counts_per_rev_) / DT_S;
