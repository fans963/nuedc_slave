...
// ── Encode helpers ────────────────────────────────────────────────────
// Each function uses a static FlatBufferBuilder to avoid malloc in ISR.
// Each builder is only called from one context (one ISR or main loop),
// so no reentrancy issue.

template <typename W>
void encode_can_rx(W &writer, uint8_t can_idx, uint32_t can_id,
                   uint8_t dlc, bool is_extended, bool is_rtr,
                   const uint8_t *data, size_t data_len)
{
    static flatbuffers::FlatBufferBuilder fbb(256);
    fbb.Clear();
...
template <typename W>
void encode_imu(W &writer, float ax, float ay, float az,
                float gx, float gy, float gz)
{
    static flatbuffers::FlatBufferBuilder fbb(256);
    fbb.Clear();
    auto frame = Protocol::SlaveToHost::CreateSlaveToHostFrame(
        fbb, Protocol::SlaveToHost::MsgPayload::ImuPack,
        Protocol::SlaveToHost::CreateImuPack(
            fbb, ax, ay, az, gx, gy, gz).Union());
    fbb.FinishSizePrefixed(frame);
...
template <typename W>
void encode_uart_rx(W &writer, uint8_t uart_idx,
                    const uint8_t *data, size_t len)
{
    static flatbuffers::FlatBufferBuilder fbb(256);
    fbb.Clear();
    auto rx_data = fbb.CreateVector(data, len);
    auto uart_pack = Protocol::SlaveToHost::CreateUartPack(
        fbb, uart_idx, rx_data);
    auto frame = Protocol::SlaveToHost::CreateSlaveToHostFrame(
...
{
    static flatbuffers::FlatBufferBuilder fbb(128);
    fbb.Clear();
    auto frame = Protocol::SlaveToHost::CreateSlaveToHostFrame(
        fbb, Protocol::SlaveToHost::MsgPayload::EncoderPack,
        Protocol::SlaveToHost::CreateEncoderPack(
            fbb, encoder_id, velocity_rad_s).Union());
    fbb.FinishSizePrefixed(frame);
...
template <typename CanDownlink, typename UartDownlink,
          typename EncoderConfig, typename MotorCommand, typename PidConfig>
void dispatch_downlink(const uint8_t *flatbuf, size_t len,
                       CanDownlink &can0_dl, CanDownlink &can1_dl,
                       UartDownlink &uart0_dl, UartDownlink &uart1_dl,
                       EncoderConfig &&on_encoder_config,
                       MotorCommand &&on_motor_cmd,
                       PidConfig &&on_pid_config)
{
    ...
    case Protocol::HostToSlave::MsgPayload::PidConfigPack: {
        ...
        if (!pid) break;
        on_pid_config(pid->controller_id(), pid->kp(), pid->ki(), pid->kd(),
                      pid->output_min(), pid->output_max(),
                      pid->integral_min(), pid->integral_max(), pid->reset());
        break;
    }
