...
public:
    ...
    constexpr explicit Pid(const Config& cfg)
        : cfg_(cfg) { }

    // ── Configuration ──────────────────────────────────────────────────

    constexpr void set_gains(T kp, T ki, T kd) { cfg_.gains = { kp, ki, kd }; }

    constexpr void set_limits(T out_min, T out_max, T int_min, T int_max) {
    ...
    constexpr T compute_error(T error) {
        ...
        d_filtered_ = cfg_.d_filter_alpha * raw_d + (T(1) - cfg_.d_filter_alpha) * d_filtered_;
...
constexpr PidF make_pid(
    float kp, float ki, float kd, float dt, float out_min = -1e30f, float out_max = 1e30f) {
    PidF::Config cfg {
        .gains  = { kp, ki, kd },
